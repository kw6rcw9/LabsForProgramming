...
public class PremiumUser: IUser
{
    ...
    public PremiumUser(double balance, string name, int age, ILogger logger)
    {
        Logger = logger;
        Balance = balance;
        Name = name;
    ...
    public void LogIn()
    {
        Logger.Log($"Hello, dear {Name}!");
    }

    public void RunApp()
    {
        Logger.Log($"Your balance: {Balance.ToString("C", new CultureInfo("en-US"))}");
        Logger.Log($"We wish you good offers and trades!");
        if (Stocks != null)
        {
            Logger.Log($"Your cool promotions: {Stocks}");
        }
    }
